  ...
  return (
    <PageLayout containerWidth="wide" padding="desktop" maxWidth="7xl">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <PageHeader
          title="Project & Assignment Helper"
          description="AI-powered comprehensive assistance for all your academic projects"
          ...
          {!projectHelp && (
            <Card className="bg-gradient-to-br from-slate-900/80 to-slate-800/60 border-gold/40 shadow-lg mb-6">
              ...
              <CardContent className="space-y-6">
                ...
                <div>
                  <Label className="text-muted-foreground mb-2 block">Subject *</Label>
                ...
                {subjectInfo && subjectInfo.commonProjectTypes.length > 0 && (
                  <div>
                    <Label className="text-muted-foreground mb-2 block">Project Type</Label>
                ...
                <div>
                  <Label htmlFor="description" className="text-muted-foreground mb-2 block">Project Description *</Label>
                  <Textarea
                    id="description"
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="requirements" className="text-muted-foreground mb-2 block">Requirements</Label>
                    <Textarea
                      id="requirements"
                  ...
                  <div>
                    <Label htmlFor="deadline" className="text-muted-foreground mb-2 block">Deadline</Label>
                    <Input
                      id="deadline"
                      type="date"
                      value={deadline}
                  ...
                  <div>
                    <Label htmlFor="grade_level" className="text-muted-foreground mb-2 block">Grade / Class Level</Label>
                    <Input
                      id="grade_level"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <Label htmlFor="word_limit" className="text-muted-foreground mb-2 block">Word Limit</Label>
                      <Input
                        id="word_limit"
                    ...
                    <div>
                      <Label htmlFor="format" className="text-muted-foreground mb-2 block">Format</Label>
                      <Input
                        id="format"
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <Label className="text-muted-foreground">Specific Questions (Optional)</Label>
            ...
            <div className="mb-6 space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {displayHistory.map((item, idx) => {
                  ...
                  return (
                    <Card key={`${item.generatedAt}-${idx}`} className="bg-card/60 border-border">
                      <CardHeader>
                        <div className="flex items-center justify-between">
                          <CardTitle className="text-sm text-muted-foreground line-clamp-2">{item.description}</CardTitle>
                      ...
                      <CardContent className="space-y-3">
                        <p className="text-xs text-muted-foreground line-clamp-3">{item.help.overview}</p>
                        <Button
                          type="button"
                          variant="outline"
                          size="sm"
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  {subjectInfo && (
                    <>
                      ...
                      <div>
                        <h2 className="text-2xl font-bold text-foreground">{subjectInfo.name} Project Help</h2>
                        {modelUsed && <p className="text-sm text-muted-foreground">Powered by {modelUsed}</p>}
                      </div>
                    </>
                  )}
                </div>
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" className="border-border/30 text-muted-foreground" onClick={() => {
                    setProjectHelp(null);
                    setDescription('');
                    setRequirements('');
                    setDeadline('');
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab}>
                ...
                <TabsContent value="overview" className="mt-6">
                  <Card className="bg-card/50 border-yellow-500/30">
                    ...
                    <CardContent>
                      <p className="text-muted-foreground whitespace-pre-wrap mb-6">{projectHelp.overview}</p>
                      {projectHelp.breakdown && (
                        <div className="grid md:grid-cols-3 gap-4">
                          <div className="p-4 bg-card rounded-lg border border-border">
                            <Label className="text-muted-foreground mb-2 block">Difficulty</Label>
                            <Badge className="bg-yellow-500/20 text-yellow-400 border-yellow-500/50">
                              {projectHelp.breakdown.difficulty_assessment || 'Medium'}
                            </Badge>
                          </div>
                          <div className="p-4 bg-card rounded-lg border border-border">
                            <Label className="text-muted-foreground mb-2 block">Estimated Time</Label>
                            <p className="text-foreground font-semibold">{projectHelp.breakdown.estimated_time || '2-3 weeks'}</p>
                          </div>
                          <div className="p-4 bg-card rounded-lg border border-border">
                            <Label className="text-muted-foreground mb-2 block">Components</Label>
                            <p className="text-foreground font-semibold">{projectHelp.breakdown.key_components?.length || 0}</p>
                          </div>
                        </div>
                      )}
                    </CardContent>
                  ...
                  <Card className="bg-card/50 border-yellow-500/30">
                    ...
                    <CardContent>
                      {projectHelp.step_by_step_plan && projectHelp.step_by_step_plan.length > 0 ? (
                        <div className="space-y-6">
                          {projectHelp.step_by_step_plan.map((step: any, idx: number) => (
                            <div key={idx} className="relative">
                              ...
                              <div className="ml-14 p-5 bg-card rounded-lg border border-border">
                        ...
                        <p className="text-muted-foreground text-center py-8">No step-by-step plan available</p>
                      )}
                    </CardContent>
                  </Card>
                </TabsContent>
                    ...
                    <CardContent>
                      {subjectInfo && (
                        <div className="grid md:grid-cols-3 gap-4 mb-6">
                          <div>
                            <Label className="text-muted-foreground mb-2 block">Websites</Label>
                            {subjectInfo.resources.websites.map((site, idx) => (
                              <div key={idx} className="p-2 bg-card rounded border border-border text-sm text-muted-foreground mb-2">
                                {site}
                              </div>
                            ))}
                          </div>
                          <div>
                            <Label className="text-muted-foreground mb-2 block">Tools</Label>
                          ...
                          <div>
                            <Label className="text-muted-foreground mb-2 block">Platforms</Label>
                      ...
                      {projectHelp.answer_specific_questions && projectHelp.answer_specific_questions.length > 0 && (
                        <div className="mt-6">
                          ...
                          {projectHelp.answer_specific_questions.map((qa, idx) => (
                            <div key={idx} className="p-5 bg-card rounded-lg border border-border mb-4">
                              <h4 className="font-semibold text-yellow-400 mb-2">Q: {qa.question}</h4>
                              <p className="text-muted-foreground text-sm whitespace-pre-wrap">{qa.answer}</p>
                            </div>
                          ))}
                        </div>
                      )}
